...
\symbl[X]{$X$}{features/attributes}
\symbl[E]{$E$}{occurrence of an error}
\symbl[Ytilde]{$\tilde{Y}$}{observed label}
The label noise taxonomy considers three types of noise: Noisy Completely at Random (NCAR)\abbrev{NCAR}{Noisy Completely at Random, from noise taxonomy}, Noisy at Random (NAR)\abbrev{NAR}{Noisy at Random, from noise taxonomy}, and Noisy Not at Random (NNAR)\abbrev{NNAR}{Noisy Not at Random, from noise taxonomy}, as described by \cite{Frenay2014}. Figure~\ref{fig:taxonomia} presents the statistical dependency between features $X$, class $Y$, observed label $\tilde{Y}$ and the occurrence of error $E$, i.e., $E=1$ when $Y \neq \tilde{Y}$. The simplest type is Noisy Completely at Random, where the occurrence of error $E$ not depend on $X$ and $Y$, such as randomly flipping labels on a binary classification problem. In Noisy at Random, the occurrence of error $E$ depends only on $Y$, such as randomly flipping labels on binary classification with different rates for positives and negatives classes. Noisy Not at Random considers the occurrence of error $E$ depending on both $Y$ and $X$, e.g., flipping labels on binary classification with different rates for each group of instances of a certain feature.
...
\symbl[T]{$T$}{transition matrix}
\symbl[Tij]{$T_{i,j}$}{transition matrix value from class $i$ to observed label $j$}
\symbl[c]{$c$}{number of classes in $Y$}
\symbl[Ycal]{$\mathcal{Y}$}{set of all class labels}
\symbl[yi]{$y_i$}{the $i$'th class value}
Many label noise robustness methods can be found on literature, in this work we highlight the \textit{backward} and \textit{forward} loss corrections, proposed by \cite{Patrini2017} using concepts of loss factorization. Those loss correction techniques considers a NAR label noise, which is described by a row-stochastic and not necessarily symmetric across the classes transition matrix $T$ such as
\begin{equation}
...
\symbl[lvar]{$\ell$}{a loss function}
\symbl[lvarbackward]{$\ell^{\leftarrow}$}{backward loss correction}
\symbl[Tinv]{$T^{-1}$}{transition matrix inverse}
...
\symbl[Ttransp]{$T^{\top}$}{transposed transition matrix}
\symbl[lvarforward]{$\ell^{\rightarrow}$}{forward loss correction}
...
\symbl[zzGamma]{$\Gamma$}{parameter/hyperparameter space}
\symbl[zzgamma]{$\gamma$}{parameter/hyperparameter configuration}
\symbl[zzgammastar]{$\gamma^*$}{parameter/hyperparameter optimal configuration}
\symbl[zzrho]{$\rho$}{a predictive performance metric}
\symbl[zzphivar]{$\varphi$}{a fairness metric}
\abbrev{s.t.}{subject to, constrained optimization}
In this context, an interesting approach is to deal with fair machine learning as a Multi-Objective Optimization (MOO)\abbrev{MOO}{Multi-Objective Optimization} problem, where predictive performance and fairness metric are the objectives, which could be defined according Equation~\ref{def:moo}, where $\gamma$ is a parameter/hyperparameter configuration in the space $\Gamma$, $\rho: \Gamma \mapsto [0,1]$ is a model performance metric and $\varphi: \Gamma \mapsto [0,1]$ a fairness metric. The set of all optimal solutions is called Pareto front, where one objective cannot be improved without sacrificing another. In this setting there is no single $\gamma^*$ optimal solution, but a set of solutions forming a Pareto front~\citep{pareto1906manuale}.

\symbl[G]{$G$}{a objective function}
...
\symbl[zzalpha]{$\alpha$}{relative importance parameter}
...
\symbl[a]{$a$}{a sensitive feature value}
\symbl[Ta]{$T_a$}{transition matrix to sensitive value $a$}
\symbl[Taij]{$T_{a,i,j}$}{transition matrix value from fair class $i$ to available label $j$ within sensitive value $a$}
\symbl[zzDeltac]{$\Delta^{c-1}$}{the simplex in a $c$-dimensional space}
\symbl[zzpsi]{$\psi$}{a invertible link function}
\symbl[lvar]{$\ell_{\psi}$}{composite loss function}
\symbl[Xcal]{$\mathcal{X}$}{feature's domain}
...
\symbl[h]{$h$}{a machine learning model/hypothesis}
...
\abbrev{FTL}{Fair Transition Loss}
Fair Transition Loss (FTL) consists in correcting model's predictions with the same technique as forward, but taking into account the sensitive attribute value when choosing the transition matrix. In Theorem~\ref{theorem:ftl} the Fair Transition Loss is formally defined, with a guarantee about its minimizers.

\symbl[FTLpsi]{$\FTL_{\psi}$}{Fair Transition Loss to a proper composite loss $\ell_{\psi}$}
...
\symbl[d0]{$d_0$}{privileged demotion rate, Fair Transition Loss parameter}
\symbl[d1]{$d_1$}{protected demotion rate, Fair Transition Loss parameter}
\symbl[p0]{$p_0$}{privileged promotion rate, Fair Transition Loss parameter}
\symbl[p0]{$p_1$}{protected promotion rate, Fair Transition Loss parameter}
...
Our hyperparameter optimization problem consists in finding an optimal trade-off between fairness and performance, which can be described as a MOO problem, as defined in Equation~\ref{def:moo}. Here, the hyperparameter configuration is $\gamma = (d_0, p_0, d_1, p_1)$. Since the transition matrix is row stochastic these parameters are sufficient to define $T_0$ and $T_1$. We want to maximize model performance $\rho(\gamma)$ and minimize fairness metric $\varphi(\gamma)$. 
...
\abbrev{TPE}{Tree-structured Parzen Estimator Sampler}
...
\abbrev{ASO}{Almost Stochastic Order, significance test}
