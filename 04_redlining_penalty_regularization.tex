...
\symbl[r]{$r$}{Pearson's $r$ correlation coefficient}
\symbl[zzsigma]{$\sigma$}{standard deviation}
\symbl[cov]{$\mathrm{Cov}$}{covariance}
\symbl[xbold]{$\mathbf{x}$}{an arbitrary random variable}
\symbl[ybold]{$\mathbf{y}$}{an arbitrary random variable}
...
\symbl[xboldi]{$\mathbf{x}_i$}{the $i$-th observations an arbitrary sampled random variable $\mathbf{x}$}
\symbl[yboldi]{$\mathbf{y}_i$}{the $i$-th observations an arbitrary sampled random variable $\mathbf{y}$}
\symbl[rs]{$r_s$}{Spearman's $r$ rank correlation coefficient}
\symbl[rankxi]{$\mathrm{rank}(\mathbf{x}_i)$}{the rank (position) of $\mathbf{x}_i$ by ordering the sample}
\symbl[n]{$n$}{a sample size}
...
\symbl[zztau]{$\tau$}{Kendall's tau rank correlation coefficient}
\symbl[xiyi]{$(\mathbf{x}_i, \mathbf{y}_i)$}{an observation pair}
\symbl[Nc]{$N_c$}{number of concordant observation pairs in rank}
\symbl[Nd]{$N_d$}{number of discordant observation pairs in rank}
...
\symbl[zzxi]{$\xi$}{Chatterjee's xi rank correlation coefficient}
\symbl[rankyixi]{$\mathrm{rank}(\mathbf{y}_i,\mathbf{x})$}{the rank (position) of $\mathbf{y}_i$ in the ordered sequence of $\mathbf{y}$ values corresponding to the sorted $\mathbf{x}$ values}
...
\symbl[l]{$l$}{the $l$-th layer of a Multi-Layer Perceptron}
\symbl[L]{$L$}{the number layers of a Multi-Layer Perceptron}
\symbl[W]{$\mathbf{W}$}{the set of all weight matrices of a Multi-Layer Perceptron}
\symbl[Wl]{$\mathbf{W}^{(l)}$}{the weight matrix for the $l$-th layer of a Multi-Layer Perceptron}
\symbl[wlij]{$w^{(l)}_{ij}$}{the weight connecting the $i$-th input neuron to the $j$-th neuron in the $l$-th layer of a Multi-Layer Perceptron}
\symbl[Dl]{$D_l$}{input size to $l$-th Multi-Layer Perceptron layer, defining $\mathbf{W}^{(l)}$ weight matrix size }
\symbl[Hl]{$H_l$}{output size to $l$-th Multi-Layer Perceptron layer, defining $\mathbf{W}^{(l)}$ weight matrix size}
...
\abbrev{RPR}{Redlining Penalty Regularizer}
...
\begin{table}[ht]
    ...
    \toprule
     \multirow{2}{*}{\shortstack[l]{Fairness/Performance\\Metric}} & Dataset & \multirow{2}{*}{\shortstack[c]{MLP\\(baseline)}} & \multirow{2}{*}{\shortstack[l]{MLP\\L2}} & \multirow{2}{*}{\shortstack[l]{MLP\\RPR$_{r}$}} & \multirow{2}{*}{\shortstack[l]{MLP\\RPR$_{r_s}$}} & \multirow{2}{*}{\shortstack[l]{MLP\\RPR$_{\tau}$}} \\ \\
