...
Before we proceed to the main experimental results, we perform another comparison using the same methodology in order to try to tackle two issues. One can argue that the proposed penalty strategy is not effective distinguished from regular regularization factors like L2.  In this perspective, eventual advantages of this method comparing with the same model without the proposed penalty would come from the conventional effects of regularization, which can avoid model overfitting and therefore boosting the fitness values. Thus, we compare the proposed strategy not only with a MLP baseline, but also with this same MLP using a standard L2 regularization over all network layers, optimized using the same ranges available on Table~\ref{tab:hyperparameters_rpr}. 

Another question is that it would be equally beneficial using traditional correlation coefficients, such as Pearson's $\rho$,  Spearman's $\rho_s$ and Kendall's $\tau$, In order to properly answer this question, we compare the proposed penalty (refered as RPR$_\xi$) with variations using absolute values of Pearson's $\rho$,  Spearman's $\rho_s$ and Kendall's $\tau$ instead of Chatterjee's $\xi$, referd as  RPR$_\rho$,  RPR$_{\rho_s}$,  and RPR$_\tau$ respectively. 

\begin{table}[ht]
    \centering
    \caption{Almost Stochastic Order test comparing RPR$_\xi$ fitness. Values under $0.5$ (in bold) mean that RPR$_\xi$ outperforms corresponding method in such optimization scenario.} \label{tab:aso_compare_RPR_variations}
    {\footnotesize
    \begin{tabular}{llccccc}
    \toprule
     \multirow{2}{*}{\shortstack[l]{Fairness/Performance\\Metric}} & Dataset & MLP & \multirow{2}{*}{\shortstack[l]{MLP\\L2}} & \multirow{2}{*}{\shortstack[l]{MLP\\RPR$_{\rho}$}} & \multirow{2}{*}{\shortstack[l]{MLP\\RPR$_{\rho_s}$}} & \multirow{2}{*}{\shortstack[l]{MLP\\RPR$_{\tau}$}} \\ \\
...
\begin{figure}[!ht]
\centering
\caption{Fitness values of RPR optimizing MCC and multiple fairness metrics.}
...
\begin{figure}[!ht]
\centering
\caption{Fitness values of RPR optimizing Accuracy and multiple fairness metrics.}
\begin{subfigure}{.32\linewidth}
    \caption{Statistical Parity}
