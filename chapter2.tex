...
A predictor $\hat{Y}$ is counterfactually fair with respect to an attribute $A$ if, under any value of $A$ and any value of non-descendants of $A$, the distribution of $\hat{Y}$ is the same in the actual world and a counterfactual world where $A$ is set to any permissible value. Mathematically, this is expressed as:
$$
P(\hat{Y}_{A \leftarrow a'}(U) = y \mid X = x, A = a) = P(\hat{Y}(U) = y \mid X = x, A = a)
$$
