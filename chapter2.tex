...
In \cite{Kamishima2012} three primary causes of unfairness in machine learning are identified:

\begin{description}
    \item[Prejudice] refers to a statistical dependence between sensitive features (e.g., race, gender) and other variables in the model. It can be direct, indirect, or latent.
    \begin{description}
        \item[Direct Prejudice] occurs when a sensitive feature is used directly in the prediction model, making the outcomes dependent on it.
        \item[Indirect Prejudice] involves statistical dependence between a sensitive variable and a target variable.
        \item[Latent Prejudice]  arises when non-sensitive features correlate with sensitive ones, leading to biased outcomes, which is frequently refered as redlining effect.
    \end{description}
    \item[Underestimation] happens when a model has not fully converged due to insufficient training data, resulting in more unfair determinations than those present in the training sample.
    \item[Negative Legacy] results from biased sampling or labeling in the training data, such as underrepresentation of certain groups.
\end{description}

Also, the authos proposes the Prejudice Remover (PR) which is a regularizer to logistic regression models. It introduces an additional term in the loss function to penalize the model for making decisions based on sensitive features. The objective function to be minimized is available on Equation~\ref{eq:prejudice_remover}, where $\Theta$ is the model parameters, $L(D;\Theta)$ the log-likelihood, $R(D, \Theta)$ the prejudice remover regularizer,$\eta$ a regularization parameter controlling the trade-off between fairness and accuracy, and $\lambda$ a parameter for the L2 regularizer.
